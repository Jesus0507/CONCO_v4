...
$.ajax({
    ...
    success: function(direccion_segura) {
        $.ajax({
            ...
            data: {
                peticion: "Consultar_Solicitudes_Vivienda",
                id: id.value
            },
        }).done(function(datos) {
                ...
                if (result_s[i]["id_solicitud"] == id.value) {
                    ...
                    switch (result_s[i]["tipo_constancia"]) {
                        ...
                        case "No poseer vivienda":
                            ...
                            break;
                        case "Vivienda":
                            titulo_solicitud = "<em class='fas fa-plus-square'></em> Solicitud de registro de " + result_s[i]["tipo_constancia"];
                            break;
                    }
...
rechazar.onclick = function() {
    var textoSwal = "Está por rechazar la solicitud de un registro de vivienda ¿desea continuar?<br><br>";
    ...
    swal({
        ...
        if (isConfirm) {
            ...
            if (document.getElementById("text-area").value == "") {
                ...
                datos_notificacion["accion"] = "Rechazó su solicitud para registro de " + solicitante["tipo_constancia"] + " debido a :" + document.getElementById("text-area").value + ".";
                ...
                solicitante["mensaje"] = "Su solicitud para registro de " + solicitante["tipo_constancia"] + " ha sido rechazada. El motivo del rechazo es: " + motivo_rechazo;
                if (solicitante["correo"] != "No posee") {
                    document.getElementById("btn_correo").click();
                } else {
                    Direccionar("Solicitudes/Administrar/Solicitudes");
                }
            }
        }
    });
...
aprobar.onclick = function() {
    ...
    datos_notificacion["usuario_receptor"] = solicitante["cedula_persona"];
    datos_notificacion["accion"] = "Aprobó su solicitud para registro de " + solicitante["tipo_constancia"] + ".";
    solicitante["asunto"] = "Se ha aprobado su solicitud";
    solicitante["mensaje"] = "Su solicitud para registro de " + solicitante["tipo_constancia"] + " ha sido aprobada.";
    if (solicitante["correo"] != "No posee") {
        document.getElementById("btn_correo").click();
    } else {
        Direccionar("Solicitudes/Administrar/Solicitudes");
...
const vue = new Vue({
    ...
    methods: {
        enviar() {
            ...
            emailjs.send("service_rbn54tj", "template_vqh9lqb", data).then(function(response) {
                if (response.text === "OK") {
                    Direccionar("Solicitudes/Administrar/Solicitudes");
                }
            }, function(err) {
